...
def create_inventory_item(
    ...
    result = crud.create_inventory(db=db, data=data)
    audit = schemas.TicketAuditCreate(
        ...
        old_value=None,
        new_value=str(result.inventory_id if hasattr(result, 'inventory_id') else result.id)
    )
...
def get_inventory_item(
    ...
    db_item = crud.get_inventory(db, inventory_id=inventory_id)
    if not db_item:
...
def list_inventory_items(
    ...
    return crud.get_inventorys(db, skip=skip, limit=limit)
...
def update_inventory_item(
    ...
    result = crud.update_inventory(db, inventory_id=inventory_id, data=data)
    audit = schemas.TicketAuditCreate(
        ticket_id=None,
        user_id=current_user.user_id,
        change_time=datetime.now(timezone.utc),
...
def delete_inventory_item(
    ...
    result = crud.delete_inventory(db, inventory_id=inventory_id)
    if not result:
        raise HTTPException(status_code=404, detail="Inventory not found")
